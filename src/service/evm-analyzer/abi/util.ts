  ...
  switch (type) {
    ...
    default:
      if (type.startsWith('uint') || type.startsWith('int')) {
        const numValue = typeof value === 'string' || typeof value === 'number' ? BigInt(value) : value;
...
export const generateInputHash = (func: AbiFunction | ABIEvent, args: string[], decimals?: number): string => {
  let result = '';
  ...
  for (let i = 0; i < func.inputs.length; i++) {
    ...
    if (decimals && (input.type?.startsWith('uint') || input.type?.startsWith('int'))) {
    ...
    if (input.type) {
      try {
        ...
        throw new Error(`Error encoding parameter '${input.name}' of type '${input.type}': ${error}`);
      }
    }
  }

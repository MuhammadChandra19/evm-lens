...
export class ContractManager {
  ...
  async deployContract(fromAddress: string, bytecode: string, contractAddress: string, options: TraceOptions = {}): Promise<DeploymentResult & { analysis: ContractAnalysis }> {
    ...
    try {
      ...
      await this.stateManager.setCode(contractAddress, parsers.hexStringToUint8Array(bytecode));

      // Create addresses
      const fromAddr = new Address(Buffer.from(fromAddress.startsWith('0x') ? fromAddress.slice(2) : fromAddress, 'hex'));
      const toAddr = new Address(Buffer.from(contractAddress.startsWith('0x') ? contractAddress.slice(2) : contractAddress, 'hex'));
      ...
      await this.stateManager.setCode(contractAddress, parsers.hexStringToUint8Array(runtimeBytecode));

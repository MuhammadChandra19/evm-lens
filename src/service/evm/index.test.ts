...
import type { EvmRuntimeParams, Test } from './types';
...
describe('EVM', () => {
  for (const t of tests as Test[]) {
    test(t.name, async () => {
      ...
      if (typeof t.expect.stack !== 'undefined') expect(result.stack).toEqual(t.expect.stack.map((item) => parsers.HexStringIntoBigInt(item)));

      if (typeof t.expect.success !== 'undefined') expect(result.success).toEqual(t.expect.success);

      if (typeof t.expect.return !== 'undefined') expect(result.return).toEqual(t.expect.return);

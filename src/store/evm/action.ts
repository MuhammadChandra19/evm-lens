...
export const deployContractToEVM = async (
  ...
  const ownerAddress = await createAccount(owner, get, actionRecorder, shouldRecord);
  if (!ownerAddress) return null;

  const contract = new Address(Buffer.from(payload.contractAddress.slice(2), 'hex'));
  const contractAddress = await createAccount(contract, get, actionRecorder, shouldRecord);
  ...
  await fundAccount(ownerAddress, parsedBalance, get, actionRecorder, shouldRecord);
...
export const callFunction = async (tx: TxData, get: () => EVMState, actionRecorder: ActionRecorder, shouldRecord: boolean = true): Promise<ExecutionResult> => {
  try {
    const evm = get().evm;
    if (!evm) return null;

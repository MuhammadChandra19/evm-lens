...
describe("Store: EVM action", () => {
  ...
  describe("deployContractToEVM", () => {
    ...
    test("should return null when create account fail: contract account", async () => {
      ...
      MOCK_CREATE_ACCOUNT.mockResolvedValueOnce(
        new Address(Buffer.from(ownerAddress.slice(2), "hex")),
    ...
    test("should return null when failed to deploy contract", async () => {
      ...
      const fromOwnerAddress = new Address(
        Buffer.from(ownerAddress.slice(2), "hex"),
      );
      const fromContractAddress = new Address(
      ...
      MOCK_DEPLOY_CONTRACT.mockResolvedValueOnce({ success: false });
    ...
    test("should return success", async () => {
      ...
      MOCK_DEPLOY_CONTRACT.mockResolvedValueOnce({ success: true });

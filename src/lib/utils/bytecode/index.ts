 ...
 */
export const KNOWN_SELECTORS: Record<
  string,
  { name: string; signature: string; standard?: string }
> = {
  // ERC20 Standard
...
export function detectStandard(
  ...
  const erc20Matches = erc20Required.filter((sel) =>
    selectorStrings.includes(sel),
  ).length;

  // ERC721 required functions
  const erc721Required = [
    "70a08231",
    "6352211e",
    "a9059cbb",
    "23b872dd",
  ...
  const erc721Matches = erc721Required.filter((sel) =>
    selectorStrings.includes(sel),
  ).length;
  ...
  analysis.functions.forEach((func, index) => {
    ...
    if (func.name && func.signature) {
      ...
      if (KNOWN_SELECTORS[func.selector]?.standard) {
        console.log(
          `   🏷️  ${KNOWN_SELECTORS[func.selector].standard} standard`,
        );
      }
    } else {
...
export function getFunctionSelector(functionName: string): string | undefined {
  ...
  const entry = Object.entries(KNOWN_SELECTORS).find(
    ([_, info]) => info.name === functionName,
  );
  return entry?.[0];
}

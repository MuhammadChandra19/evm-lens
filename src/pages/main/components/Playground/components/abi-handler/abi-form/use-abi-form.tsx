...
const useAbiForm = () => {
  ...
  const [errors, setErrors] = useState(Array(submissionForm.length).fill("") as string[])
    ...
    for(let i = 0; i < form.length; i++) {
      ...
      if(current.type.match(/^u?int(\d+)?$/)) {
        ...
        if(!isNaN(num) && num <= 0 && !Number.isInteger(num)) {
          ...
          setErrors((err) => {
            const copyErr = err
            copyErr[i] = `${current.name} Must be a valid number (non-negative integer)`
      ...
      if(current.type === "address" && !(/^0x[a-fA-F0-9]{40}$/.test(current.value))) {
        hasError = true
        setErrors((err) => {
          const copyErr = err
          copyErr[i] = `${current.name} Must be a valid Ethereum address (0x followed by 40 hex characters)`
